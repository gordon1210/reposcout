...
/// Render the report as pretty JSON. When `summary_only` is set, the heavy
...
pub fn render(report: &ScanReport, summary_only: bool, baseline_ready: bool) -> Result<String> {
    if summary_only || baseline_ready {
        ...
        return serde_json::to_string_pretty(&value).context("failed to render scan report JSON");
    }
    serde_json::to_string_pretty(report).context("failed to render scan report JSON")
}
