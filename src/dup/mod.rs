...
#[derive(Debug, Clone, Copy)]
pub(crate) struct DetectionThresholds {
    tokens: usize,
    lines: usize,
    similarity: f64,
}
...
impl DetectionThresholds {
    pub(crate) fn new(min_tokens: usize, min_lines: usize, min_similarity: f64) -> Self {
        Self {
            tokens: min_tokens,
            lines: min_lines,
...
fn format_pool(format: &str, scope: DuplicationFormatScope) -> String {
    match scope {
        ...
        DuplicationFormatScope::Exact | DuplicationFormatScope::Compatible => format.to_string(),
