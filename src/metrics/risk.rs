...
#[must_use]
pub fn explain(file: &FileReport) -> RiskExplanation {
    let sloc = file.sloc;
    let cyclomatic = file
        .complexity
        .as_ref()
...
#[must_use]
pub fn entry(file: &FileReport) -> Option<RiskEntry> {
    let risk = explain(file);
...
mod tests {
    ...
    fn saturation_anchors_are_half_saturation_points() {
        let risk = explain(&file_with("src/anchor.rs", 1_000, 100, 20));
    ...
    fn risk_remains_monotonic_above_the_former_saturation_anchors() {
        let first = explain(&file_with("src/first.rs", 1_000, 100, 20));
        let second = explain(&file_with("src/second.rs", 2_000, 200, 40));
        let third = explain(&file_with("src/third.rs", 4_000, 400, 80));
    ...
    fn compact_and_detailed_risk_evidence_identify_the_algorithm() {
        ...
        let explanation = explain(&file);
        let compact = entry(&file).expect("risk entry");
    ...
    #[test]
    fn risk_json_contains_no_filename_based_test_claims() {
        let detailed = serde_json::to_value(explain(&representative_file())).unwrap();
        let compact = serde_json::to_value(entry(&representative_file()).unwrap()).unwrap();

        for value in [detailed, compact] {
            assert!(value.get("untested").is_none());
            assert!(value.get("untested_multiplier").is_none());
            assert!(
                value["reasons"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .all(|reason| !reason.as_str().unwrap().contains("test"))
            );
        }
    }
