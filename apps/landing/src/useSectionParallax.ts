...
function writeOffsets(
  ...
  const normalizedDistance = clamp(
    distanceFromCenter / Math.max(viewportHeight, 1),
    1
  )
  ...
  element.style.setProperty(
    "--parallax-slow-reverse",
    `${(-slow).toFixed(2)}px`
  )
  element.style.setProperty(
    "--parallax-medium-reverse",
    `${(-medium).toFixed(2)}px`
  )
...
export function useSectionParallax() {
  useEffect(() => {
    ...
    const sections = Array.from(
      document.querySelectorAll<HTMLElement>(parallaxSections)
    )
    ...
    const update = () => {
      ...
      if (reducedMotion.matches) {
        sections.forEach((section) =>
          writeOffsets(section, 0, viewportHeight, 0)
        )
        return
      }

      const intensity =
        window.innerWidth <= 660 ? 0.52 : window.innerWidth <= 900 ? 0.75 : 1
